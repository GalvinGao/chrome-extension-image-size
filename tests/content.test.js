...
test('file-size badge expands details, flags density, updates dimensions and cleans up', async () => {
  ...
  assert.equal(details.children[7].children[1].textContent, '6.00 MB / MP · high');
  ...
  displayWidth = 40;
  resized(); frames.shift()();
  assert.equal('heavy' in host.dataset, true, 'Collapsed badge reacts to displayed area changes');
  displayWidth = 0;
  resized(); frames.shift()();
  assert.equal('heavy' in host.dataset, false, 'Zero displayed area has no density');
