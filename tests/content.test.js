...
test('file-size badge expands details, flags density, updates dimensions and cleans up', async () => {
  ...
  assert.equal(host.dataset.density, 'warning', 'Exactly 6 MB/MP is orange');
  assert.equal('expanded' in host.dataset, false);
  assert.match(style.textContent, /\.details \{ display:none/);
  assert.match(style.textContent, /data-density="high".*background:#a32222/);
  assert.match(style.textContent, /data-density="warning".*background:#9a420d/);
  ...
  assert.equal(details.children[7].children[1].textContent, '6.00 MB / MP · warning');
  ...
  assert.equal(details.children[7].children[1].textContent, '7.50 MB / MP · warning');
  img.srcset = '';
  picture = { querySelectorAll: () => [{ getAttribute: () => 'a 2x' }] };
  onMessage({ type: 'sizes', results: [[img.src, { ...result, bytes: 12000 }]] }); frames.shift()();
  assert.equal(host.dataset.density, 'warning', 'Exactly 3 MB/MP is orange');
  assert.equal(details.children[8].hidden, false);
  host.events.pointerleave();
  assert.equal('expanded' in host.dataset, false);
  displayWidth = 40;
  resized(); frames.shift()();
  assert.equal(host.dataset.density, 'warning', 'Exactly 6 MB/MP remains orange while collapsed');
  displayWidth = 30;
  resized(); frames.shift()();
  assert.equal(host.dataset.density, 'warning', 'Exactly 8 MB/MP is orange');
  displayWidth = 29;
  resized(); frames.shift()();
  assert.equal(host.dataset.density, 'high', 'Above 8 MB/MP is red');
  displayWidth = 81;
  resized(); frames.shift()();
  assert.equal(host.dataset.density, undefined, 'Below 3 MB/MP is neutral');
  displayWidth = 0;
  resized(); frames.shift()();
  assert.equal('density' in host.dataset, false, 'Zero displayed area has no density');
  assert.equal(details.children[7].children[1].textContent, '—');
  displayWidth = 80;
  ...
  assert.equal('density' in host.dataset, false);
  onMessage({ type: 'sizes', results: [[img.src, result]] }); frames.shift()();
  onMessage({ type: 'settings', settings: { badge:'transfer', colors:false, mime:false, intrinsic:false, srcset:false, density:false } }); frames.shift()();
  assert.equal(size.textContent, '0 B');
  assert.equal(host.dataset.density, undefined);
